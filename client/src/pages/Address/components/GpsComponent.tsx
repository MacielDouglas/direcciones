...
const isValidCoordPair = (value: string) =>
  /^-?\d{1,2}\.\d+\s*,\s*-?\d{1,3}\.\d+$/.test(value.trim());
...
const GpsInput = ({ setFormData, formData }: GpsInputProps) => {
  ...
  useEffect(() => {
    if (formData.gps) {
      const [pastedLat, pastedLng] = formData.gps
        .split(",")
        .map((s) => s.trim());
      setLat(pastedLat);
      setLng(pastedLng);
  ...
  const handlePaste = async () => {
    try {
      ...
      if (!isValidCoordPair(clipboardText)) {
        setError("Formato inválido. Use: -8.508111, -35.008334");
        return;
      }

      const [pastedLat, pastedLng] = clipboardText
        .split(",")
        .map((s) => s.trim());
      setLat(pastedLat);
      setLng(pastedLng);
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          onClick={handleClear}
          disabled={!lat && !lng}
          className="px-4 py-2 bg-tertiary-drk text-white rounded-lg text-sm disabled:opacity-50"
        >
          Limpar
        </button>
      </div>

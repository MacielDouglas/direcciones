...
function Card() {
  ...
  const [cardToReturn, setCardToReturn] = useState(null);
  const { returnedCardInput } = useCard();
          ...
          >
            <div className="flex flex-col gap-4 p-6 w-full">
              <div className="flex items-center justify-between flex-wrap">
                ...
                <p className="text-sm w-full flex items-center gap-2 justify-end">
                  <FaLocationDot /> {card.street.length}
                  {card.street.length <= 1 ? " dirección" : " direcciones"}
                </p>
              </div>
              <button
                className="px-2 py-1 bg-secondary text-white rounded text-sm"
                onClick={() => {
                  setCardToReturn(card.id);
            ...
            <ul className="space-y-4 p-2">
              {card.street.map((street, index) => {
                ...
                return (
                  ...
                  >
                    <button
                      onClick={() => setSelectedAddress(street.id)}
                      className="flex flex-col w-full text-left"
                    >
                      <div className="flex items-center justify-between w-full ">
                        <p className="text-4xl">
                          {(street.type === "house" && <MdHouse />) ||
                            (street.type === "department" && (
                              <MdOutlineApartment />
                            )) ||
                            (street.type === "store" && (
                              <MdOutlineStorefront />
                            )) ||
                            (street.type === "restaurant" && (
                              <MdRestaurant />
                            )) ||
                            (street.type === "hotel" && <MdHotel />)}
                        </p>
                        <p className="text-sm text-stone-500 flex items-center gap-1">
                        ...
                        <div className="w-full justify-between flex gap-5  ">
                          <div>
                            ...
                            <p className="text-gray-500 text-sm">
                              {street.city}, {street.neighborhood},
                            </p>
                            {street.complement && (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center w-full h-full p-4">
          <div className="relative bg-white rounded-lg shadow-lg max-w-lg w-full p-8">
            <button
              ...
              className="absolute -top-5 right-5 bg-secondary text-2xl text-slate-300 p-2 rounded-full border border-red-800"
            >
              <RiCloseLargeFill />
            </button>
...
//               const address = addressesData?.addressesData?.find(
//                 (address) => address.id === streetId
//               );
...
//   const myCards = cardsData.cardsData;
//   const addressesData = useSelector((state) => state.addresses);

//   console.log(myCards);
//   console.log("lista de cartoes", cardsData);

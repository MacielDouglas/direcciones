...
const FULL_CARD_UPDATED = "FULL_CARD_UPDATED";
...
const cardResolver = {
  Query: {
    getCard: async (_, __, { req }) => {
      ...
      try {
        const cardsList = await Card.find({}).lean();
        // Coletar todos os IDs de endereços dos cards de forma eficiente
        const streetIds = [
          ...new Set(cardsList.flatMap((card) => card.street)),
        ];
      ...
      //     ...card,
      //     id: card._id.toString(),
      //     user: findUserById(card.userId),
      //   }));
      //   return {
      //     message: `Encontrado ${cards.length} tarjetas`,
      //     success: true,
      //     cards,
      //   };
    },
  ...
  Mutation: {
    ...
    updateCard: async (_, { id, input }, { req }) => {
      verifyAuthorization(req);
      validateObjectId(id);
      const updatedCard = await Card.findByIdAndUpdate(id, input, {
        new: true,
      });
      if (!updatedCard) throw new Error("Cartão não encontrado.");
      await notifyFullCardUpdate();
      return {
    ...
    assignCard: async (_, { input }, { req }) => {
      ...
      const updatedCards = await Promise.all(
        cardIds.map(async (id) => {
          ...
          if (card.startDate) {
            const assignedUser = card.usersAssigned.find(
              (u) => u.userId.toString() === userId
            );
            if (assignedUser) {
              throw new Error(
                `O cartão ${id} já está designado para este usuário.`
              );
            } else {
              throw new Error(
                `O cartão ${id} está designado para outro usuário.`
              );
            }
          }
    ...
    returnCard: async (_, { input }, { req }) => {
      ...
      if (!card.startDate) {
        throw new Error("Este cartão já foi devolvido.");
      } else {
        ...
        return {
          ...
          card: { ...cardUpdate, id: cardUpdate._id.toString() },
        };
      }
    },
  ...
  Subscription: {
    fullCard: {
      subscribe: () => pubsub.asyncIterator([FULL_CARD_UPDATED]),
    },
  },
};

const notifyFullCardUpdate = async () => {
  ...
  const updatedCards = cards.map((card) => {
    const updatedStreets = card.street
      .map((streetId) => {
        const address = addressMap.get(streetId.toString());
        return address ? { ...address, id: address._id.toString() } : null;
      })
      .filter(Boolean); // Filtra diretamente valores null ou undefined
  ...
  pubsub.publish(FULL_CARD_UPDATED, {
    fullCard: {
      success: true,
      message: "Lista de cartões atualizada.",
      cards: updatedCards,
    },
  });
};

export { cardResolver, notifyFullCardUpdate };

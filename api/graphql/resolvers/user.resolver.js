  ...
  Query: {
    getUser: async (_, { id }, { req, res }) => {
      ...
      try {
        ...
        return {
          success: true,
          message: `Usuario: ${user.name}, encontrado.`,
          user: {
            ...sanitizeUser(user),
          },
        };
    ...
    getUsers: async (_, __, { req }) => {
      ...
      try {
        ...
        const users = await User.find(
          filter,
          "id name group codUser profilePicture"
        );
  ...
  Mutation: {
    loginWithGoogle: async (_, { user }, { res }) => {
      ...
      try {
        ...
        if (!existUser) {
          ...
          const uniqueNumber = await hashToNumbers(uid);
    ...
    deleteUser: async (_, { id }, { req, res }) => {
      ...
      if (!decodedToken || !decodedToken.isSS) {
        throw new Error("No tienes permiso para deletar usuarios.");
      }
    ...
    updateUser: async (_, { action, id, updates }, { req, res }) => {
      const decodedToken = verifyAuthorization(req);
      if (!decodedToken || !decodedToken.isSS) {
        throw new Error("No tienes permiso para cambiar usuarios.");
      }
      ...
      switch (action) {
        case "addGroup":
          try {
            ...
            if (decodedToken.group === userToUpdate.group) {
              ...
              const updateUser = await User.findByIdAndUpdate(id, userUpdate, {
                new: true,
              });

              return {
                success: true,
                message: `El usuario ${updateUser.name}, ha sido eliminado del grupo y sus informaciónes fue borrada.`,
            ...
            if (userToUpdate.group !== "0") {
              throw new Error(
                "este ususario ya pertenece a otro grupo. Primero, elimínelo del grupo actual..."
              );
            }
        ...
        case "designateIss":
          try {
            ...
            if (userToDesignate.isSS) {
              userToDesignate.isSS = false;
              await userToDesignate.save();

              return {
                success: true,
                message: `El usuario ${userToDesignate.name} ya no es SS.`,
